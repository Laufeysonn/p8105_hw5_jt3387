...
unsolved homicides and Chicago, IL has the highest rate among these 50
cities.

## Problem 3

Create a function to save the estimated mean and the p-value from the
t-test, and apply it to a simulation containing 5000 times t-test with =
0 by using `map` function.

``` r
ttest_simulation <- function(n_obs = 30, mu, sigma = 5) {
...
result_mu_0 <- expand_grid(mu = 0, iter = 1:5000) %>% 
  mutate(test_result = map(.x = mu, ~ttest_simulation(mu = .x))) %>% 
  unnest(test_result)
head(result_mu_0, 10)
```
...
results_allmu %>%
  ...
  geom_point() + 
  labs(title = "Null Hypothesis Rejection Proportion and True Mean",
       x = "True Mean",
...
-   From the plot, we can see that the power increases as the true
    sample mean increases, so there is a positive relationship between
    the effect size and power.
...
ggplot(hat_mu_all, aes(x = mu, y = hat_mu, color = 'blue')) +
  ...
  labs(title = "Average Mean Estimate by True Mean",
       x = "True Mean",
       y = "Average Mean Esimate")
```

<img src="p8105_hw5_jt3387_files/figure-gfm/unnamed-chunk-12-1.png" width="90%" />

-   From the plot, we can see that the sample average of ｛｝ across
    tests for which the null is rejected is approximately equal to the
    true value of when the true mean equals to 0, 4, 5, and 6, while
    they are not approximately equal to each other when the true mean
    equals to 1, 2, and 3.
...
-   The estimated in non-reject sample is basically small, and the
    ...
    reject sample is not a big part of the whole sample. Therefore, when
    we get rid of the non-reject sample, the in the reject sample is
    going to get bigger. When = 4, 5, 6, the effect size and null
    rejection proportion (power) is big, so the reject sample is a big
    part of the whole sample. Therefore, the in the reject sample are
    similar to the true mean after getting rid of the non-reject sample.
